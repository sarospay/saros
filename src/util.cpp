...
const char * const BITCOIN_CONF_FILENAME = "saros.conf";
const char * const BITCOIN_PID_FILENAME = "sarosd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "saros" is a composite category enabling all Saros-related debug output
            if(ptrCategory->count(string("saros"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SarosCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SarosCore
    // Mac: ~/Library/Application Support/SarosCore
    // Unix: ~/.saroscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SarosCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SarosCore";
#else
    // Unix
    return pathRet / ".saroscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty saros.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override saros.conf
